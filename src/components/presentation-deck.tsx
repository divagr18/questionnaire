...
export function PresentationDeck({ pack: initialPack }: { pack: Pack }) {
  ...
  return <main className={`deck accent-${pack.accent}`}>
    ...
    <footer className="deck-controls"><div className="deck-progress"><span style={{ width: `${pack.queue.length ? ((slide + 1) / total) * 100 : 0}%` }} /></div><div className="deck-control-row"><span>{pack.queue.length ? `${slide + 1} / ${pack.queue.length}` : "0 / 0"}</span><div><button aria-label="Previous slide" disabled={slide === 0} onClick={() => move(slide - 1)}><ArrowLeft /></button><button aria-label="Next slide" disabled={slide >= pack.queue.length - 1} onClick={() => move(slide + 1)}><ArrowRight /></button><button aria-label="Toggle fullscreen" onClick={() => fullscreen ? void document.exitFullscreen() : void document.documentElement.requestFullscreen()}>{fullscreen ? <Minimize /> : <Expand />}</button></div></div></footer>
  </main>;
}
